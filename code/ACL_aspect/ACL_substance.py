...
# print(data.SUBSTANCE.value_counts())  # 1:8, 2:22, 3:58, 4:145, 5:17

# df1 = data[data['SUBSTANCE'].isin([1])]
# df1 = df1.sample(n=6, replace=False, random_state=None, axis=0)
# df2 = data[data['SUBSTANCE'].isin([2])]
# df2 = df2.sample(n=12, replace=False, random_state=None, axis=0)
# df3 = data[data['SUBSTANCE'].isin([3])]
# df3 = df3.sample(n=28, replace=False, random_state=None, axis=0)
# df4 = data[data['SUBSTANCE'].isin([4])]
# df4 = df4.sample(n=45, replace=False, random_state=None, axis=0)
# df5 = data[data['SUBSTANCE'].isin([5])]
# df5 = df5.sample(n=9, replace=False, random_state=None, axis=0)
...
data = pd.read_csv('../../data/Peerread/SUBSTANCE/sub_al-0.533-0.227.csv')


# get train/test data
x_train = data['comments'][:TRAIN_NUM]
y_train = data['SUBSTANCE'][:TRAIN_NUM]
x_test = data['comments'][TRAIN_NUM:]
y_test = data['SUBSTANCE'][TRAIN_NUM:]
...
# x_train_unlabeled = pd.read_csv('../../data/OpenReview_Data/ICLR_Review_all_processed.csv')
# x_train_unlabeled = x_train_unlabeled['text'][:UNLABELED_NUM]
# x_train_unlabeled = vectorize_sequences_bert(x_train_unlabeled)
...
model = LD(layer_sizes=[inp_size, 768, 700, n_classes])  # 768,768,300,6
...
# model.save_weights(f"../../data/Peerread/SUBSTANCE/lf{accuracy:.3f}-{f1score:.3f}.h5")
# data.to_csv(f'../../data/Peerread/SUBSTANCE/sub_lf-{accuracy:.3f}-{f1score:.3f}.csv', sep=',', index=False)
