...
TRAIN_NUM = 100
data = pd.read_csv('../../data/SemEval/ambience/100-0.7300-0.4879.csv')
# data = pd.read_csv('../../data/SemEval/SemEval14_ambience_no_conflict.csv')
# print(data.label.value_counts())
#
# df1 = data[data['label'].isin([1])]
# df1 = df1.sample(n=35, replace=False, random_state=None, axis=0)
# df2 = data[data['label'].isin([2])]
# df2 = df2.sample(n=10, replace=False, random_state=None, axis=0)
# df3 = data[data['label'].isin([3])]
# df3 = df3.sample(n=55, replace=False, random_state=None, axis=0)
...
# data_for_test = data.drop_duplicates(
#     subset=['text', 'AC', 'ACP', 'label'],
#     keep=False)
# df4 = data_for_test[data_for_test['label'].isin([1])]
# df4 = df4.sample(n=35, replace=False, random_state=None, axis=0)
# df5 = data_for_test[data_for_test['label'].isin([2])]
# df5 = df5.sample(n=10, replace=False, random_state=None, axis=0)
# df6 = data_for_test[data_for_test['label'].isin([3])]
# df6 = df6.sample(n=55, replace=False, random_state=None, axis=0)
# data_for_test = pd.concat([df4, df5, df6], ignore_index=True)
# data_for_test = shuffle(data_for_test)
# data1 = pd.concat([data_for_train, data_for_test], ignore_index=True)
...
# x_train_unlabeled = data_for_unlabel['text'][:200]
...
# data = pd.concat([data_for_train, data_for_test, data_for_unlabel], ignore_index=True)
# data.to_csv(f'../../data/SemEval/ambience/100-{result[0][0]:.4f}-{result[0][1]:.4f}.csv', sep=',', index=False)
...
# f1score = f1_score(y_test_new, y_pred, average='macro')
