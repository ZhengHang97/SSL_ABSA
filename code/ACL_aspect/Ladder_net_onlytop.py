...
def LD(layer_sizes=[],  # 768,768,300,6
                          noise_std=0.3,
                          denoising_cost=[1.0, 0, 0, 0]):  # , 0.10, 0.10, 0.10
    L = len(layer_sizes)-1  # number of layers:3

    #  task1
    inputs_l = Input(shape=(layer_sizes[0],))  # 预期的输入是768维的
    ...
    def encoder(inputs, noise_std):
        ...
        all_z = [None for _ in range(len(layer_sizes))]  # 初始化所有z
    ...
    for l in range(L, -1, -1):  # 从3开始到0结束，每次减1,（3,2,1,0）
